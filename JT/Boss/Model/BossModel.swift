...
class BossModel: NSObject {
    var id:String = ""
    var name:String = ""
    var isSelected:Bool = false
}

class BossSectionModel:NSObject{
    var sectionTitle:String?
    var values:[BossModel] = []
    var selectValue:[BossModel]{
        get{
    ...
    func containModel(model:BossModel) -> Bool{
        var flag = false
    ...
    var openViewFrameArr: [NSValue] {
        get{
            ...
            for item in values {
                ...
                let frame = CGRect.init(x: x, y: y, width: w, height: filterOpenH)
                frameArr.append(NSValue(cgRect: frame))
                x += (w+filterCloseLeft)
